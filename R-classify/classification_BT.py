...
# 构建带有 L2 正则化和双向 LSTM 的模型
model = tf.keras.Sequential([
    tf.keras.layers.LSTM(256, activation='relu', input_shape=(num_peaks, 1),
                         return_sequences=True, kernel_regularizer=regularizers.l2(0.001)),
    tf.keras.layers.Dropout(0.4),
    tf.keras.layers.LSTM(128, activation='relu', kernel_regularizer=regularizers.l2(0.001)),
    tf.keras.layers.Dense(128, activation='relu', kernel_regularizer=regularizers.l2(0.001)),
    tf.keras.layers.Dropout(0.3),
    tf.keras.layers.Dense(len(dataframes), activation='softmax', kernel_regularizer=regularizers.l2(0.001))
])
# model = tf.keras.Sequential([
#     Bidirectional(tf.keras.layers.LSTM(256, activation='relu', input_shape=(num_peaks, 1),
#                                        return_sequences=True, kernel_regularizer=regularizers.l2(0.01))),
#     tf.keras.layers.Dropout(0.4),
#     Bidirectional(tf.keras.layers.LSTM(128, activation='relu', kernel_regularizer=regularizers.l2(0.01))),
#     tf.keras.layers.Dense(128, activation='relu', kernel_regularizer=regularizers.l2(0.01)),
#     tf.keras.layers.Dropout(0.3),
#     tf.keras.layers.Dense(len(dataframes), activation='softmax', kernel_regularizer=regularizers.l2(0.01))
# ])
